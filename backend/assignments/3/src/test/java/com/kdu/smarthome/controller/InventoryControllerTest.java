    ...
    public static void displayInventory(MockMvc mockMvc, String... args) throws Exception {
        try {
            ...
            mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/inventory")
                    ...
                    .andExpect(MockMvcResultMatchers.jsonPath("$.inventory.kickston_id").value(Matchers.containsString(kickstoneId)))
                    .andExpect(MockMvcResultMatchers.jsonPath("$.inventory.device_username").value(Matchers.containsString(deviceName)))
                    .andExpect(MockMvcResultMatchers.jsonPath("$.inventory.device_password").value(Matchers.containsString(devicePassword)))
                    .andDo(result -> {
